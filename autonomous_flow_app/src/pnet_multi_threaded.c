...
#define MT_MQTT_KEEPALIVE_S     120
...
/*
 * MT_CUSTOMER_REPRO = 1 reproduces the customer's ordering and timing
 *  "2 HTTP requests" failure):
 *   1. Power save is enabled BEFORE the Wi-Fi connect, not after DHCP.
 *   2. HTTPS starts in its own thread at the exact moment DHCP_BOUND is
 *      delivered (customer: iConfig GET in the same ms as the DHCP event),
 *      racing the driver's DNS server configuration.
 *   3. No artificial settle delays between DHCP, HTTPS and MQTT.
 *   4. Power save is not toggled off on MQTT loss.
 * HTTPS details (WAN servers, async DNS, socket service, retries) are in
 * https_get.c (CUSTOMER_REPRO).
 */
#define MT_CUSTOMER_REPRO       1
#define MT_BOOT_DELAY_S         (MT_CUSTOMER_REPRO ? 2 : 8)
...
K_THREAD_STACK_DEFINE(mt_http_stk,    MT_HTTP_STACK);
...
/* ========================================================================
 * Thread 4: mt_http (prio 7) - customer-like iConfig GET + onboarding POST
 * Starts HTTPS the moment DHCP_BOUND is delivered, like the customer's
 * gorm_iconfig / be_mqtt_onboarding modules.
 * ======================================================================== */
static void mt_http_thread(void *p1, void *p2, void *p3)
{
ARG_UNUSED(p1); ARG_UNUSED(p2); ARG_UNUSED(p3);

printk("[MT-HTTP] HTTPS thread started (waits for DHCP_BOUND)\n");

while (1) {
(void)k_event_wait(&mt_dhcp_event, 1U, false, K_FOREVER);
...
printk("[MT-HTTP] DHCP bound -> iConfig-like HTTPS GET now\n");
rc = https_phase1_get(&mt_https_mqtt_cfg);
printk("[MT-HTTP] GET result %d after %lld ms (cfg %s)\n", rc,
       k_uptime_get() - t0, mt_https_mqtt_cfg.valid ? "valid" : "defaults");

t0 = k_uptime_get();
printk("[MT-HTTP] Onboarding-like HTTPS POST now\n");
rc = https_onboard_post_probe(&mt_https_mqtt_cfg);
printk("[MT-HTTP] POST result %d after %lld ms\n", rc, k_uptime_get() - t0);
...
k_sleep(K_SECONDS(MT_BOOT_DELAY_S)); /* Boot delay */
...
#if MT_CUSTOMER_REPRO
/* Customer order: power save ON before association. */
if (!mt_in_ps) {
printk("[MT-FLOW] Customer order: enabling Power Save BEFORE WiFi connect\n");
(void)mt_ps_enable(true);
}
#endif
...
#if MT_CUSTOMER_REPRO
	/* ─── Phase 2–3: HTTPS runs in mt_http (started at DHCP_BOUND) ── */
	while (!mt_https_phase_done && mt_wifi_connected) {
		k_msleep(100);
	}
	if (!mt_wifi_connected) {
		continue;
...
#else
       /* ─── Phase 2–3: PS on, then two HTTPS requests ────────── */
	{
		static bool https_phase1_done;

              if (!mt_in_ps) {
                     printk("[MT-FLOW] Phase 2: Enabling Power Save before HTTPS\n");
                     mt_ps_enable(true);
                     k_msleep(300);
              }

		if (!https_phase1_done) {
                     printk("[MT-FLOW] Phase 3: HTTPS GET MQTT cfg + certs...\n");
			rc = https_phase1_get(&mt_https_mqtt_cfg);
			if (rc != 0) {
				printk("[MT-FLOW] HTTPS GET failed: %d (MQTT defaults)\n",
				       rc);
			} else if (mt_https_mqtt_cfg.valid) {
                            printk("[MT-FLOW] Phase 3: GET cfg OK %s:%u id=%s certs=%s\n",
				       mt_https_mqtt_cfg.host,
				       mt_https_mqtt_cfg.port,
				       mt_https_mqtt_cfg.client_id,
				       mt_https_mqtt_cfg.certs_valid ? "yes" : "flash");
			} else {
                            printk("[MT-FLOW] Phase 3: GET OK, parse miss (defaults)\n");
                     }

                     printk("[MT-FLOW] Phase 3: HTTPS POST onboarding probe...\n");
                     rc = https_onboard_post_probe(&mt_https_mqtt_cfg);
                     if (rc != 0) {
                            printk("[MT-FLOW] HTTPS POST probe failed: %d\n", rc);
                     } else {
                            printk("[MT-FLOW] Phase 3: HTTPS POST probe finished\n");
			}
...
printk("[MT-FLOW] Phase 5: MQTT connected\n");
       ...
       /* ─── Phase 6: Subscribe once ────────────────────────── */
if (!mt_mqtt_subscribed) {
       printk("[MT-FLOW] Phase 6: Subscribing '%s'\n",
       MT_SUB_TOPIC);
...
#if MT_CUSTOMER_REPRO
mt_http_tid = k_thread_create(&mt_http_data, mt_http_stk,
              K_THREAD_STACK_SIZEOF(mt_http_stk),
              mt_http_thread, NULL, NULL, NULL,
              K_PRIO_PREEMPT(7), 0, K_NO_WAIT);
k_thread_name_set(mt_http_tid, "mt_http");
printk("[MT] 5 threads started: mt_flow | mt_mqtt_poll | mt_publish | mt_udp | mt_http\n");
#else
