...
static int http_request_socket_set_blocking(int socket_id, bool blocking)
{
	int flags = zsock_fcntl(socket_id, F_GETFL, 0);

	if (flags < 0) {
		printk("[HTTPS] fcntl(F_GETFL) failed: %d (errno=%d)\n", flags, errno);
		return -errno;
	}

	int target_flags = blocking ? (flags & ~O_NONBLOCK) : (flags | O_NONBLOCK);
	int rc = zsock_fcntl(socket_id, F_SETFL, target_flags);

	if (rc < 0) {
		printk("[HTTPS] fcntl(F_SETFL, %s) failed: %d (errno=%d)\n",
		       blocking ? "BLOCKING" : "O_NONBLOCK", rc, errno);
		return -errno;
	}

	printk("[HTTPS] fcntl(F_SETFL, %s) succeeded\n",
	       blocking ? "BLOCKING" : "O_NONBLOCK");
	return 0;
}
...
static void zephyr_http_cb(struct http_response *rsp,
                           enum http_final_call final_data,
                           void *user_data)
{
	int *total_recv = (int *)user_data;

	if (rsp->data_len > 0 && total_recv != NULL) {
		int space = (int)sizeof(response_buffer) - 1 - *total_recv;

		if (space > 0) {
			int copy_len = MIN((int)rsp->data_len, space);

			memcpy(response_buffer + *total_recv, rsp->recv_buf, copy_len);
			*total_recv += copy_len;
...
int https_phase1_get(struct https_mqtt_cfg *cfg)
{
	...
	printk("[HTTPS] TLS connected — switching socket to blocking mode for Zephyr internal http_client_req()\n");
	http_request_socket_set_blocking(ctx.socket, true);

	static uint8_t http_chunk_buf[1024];
	total_recv = 0;
	...
	req.method = HTTP_GET;
	req.url = HTTPS_PATH;
	req.host = HTTPS_SERVER;
	req.protocol = "HTTP/1.1";
	req.response = zephyr_http_cb;
	req.recv_buf = http_chunk_buf;
	req.recv_buf_len = sizeof(http_chunk_buf);

	int http_rc = http_client_req(ctx.socket, &req, 5000, &total_recv);

	if (http_rc < 0) {
		printk("[HTTPS] Zephyr http_client_req failed: %d\n", http_rc);
		ret = http_rc;
		goto out;
	}
	...
	ret = 0;
	printk("[HTTPS] Phase 4 GET completed successfully using Zephyr http_client_req()\n");
