...
static bool             mt_fw_version_queried = false;
...
static int mt_query_fw_version(void)
{
...
if (iface == NULL) {
printk("[MT] No default WiFi interface for version query\n");
return -ENODEV;
}

rc = net_mgmt(NET_REQUEST_WIFI_VERSION, iface, &version, sizeof(version));
if (rc != 0) {
printk("[MT] Failed to get WiFi version (%d)\n", rc);
return rc;
}

printk("[MT] WiFi driver version: %s\n",
       version.drv_version ? version.drv_version : "unknown");
printk("[MT] WiFi firmware version: %s\n",
...
printk("MQTT subscribe failed: topic=%s qos=%d (total %lld ms, rc=%d)\n",
       topic, MQTT_QOS_1_AT_LEAST_ONCE, k_uptime_get() - start_ms, rc);
return rc;
}

tx_ms = k_uptime_get() - start_ms;
printk("[MQTT-TIME] subscribe tx: %lld ms\n", tx_ms);

int wait_rc = mt_mqtt_wait_flag(&mt_mqtt_suback_rx, 10000);
wait_ms = k_uptime_get() - start_ms - tx_ms;
printk("[MQTT-TIME] SUBACK wait: %lld ms (rc=%d)\n", wait_ms, wait_rc);

if (wait_rc == 0) {
printk("MQTT subscribe successful: topic=%s qos=%d (total %lld ms)\n",
       topic, MQTT_QOS_1_AT_LEAST_ONCE, k_uptime_get() - start_ms);
return 0;
}

printk("MQTT subscribe failed: topic=%s qos=%d (total %lld ms, rc=%d)\n",
       topic, MQTT_QOS_1_AT_LEAST_ONCE, k_uptime_get() - start_ms, wait_rc);
return wait_rc;
}
...
printk("MQTT publish failed: topic=%s qos=%d (total %lld ms, rc=%d)\n",
       topic, p.message.topic.qos, k_uptime_get() - start_ms, rc);
return rc;
}
...
int wait_rc = mt_mqtt_wait_flag(&mt_mqtt_puback_rx, 10000);
wait_ms = k_uptime_get() - start_ms - tx_ms;
printk("[MQTT-TIME] PUBACK wait: %lld ms (rc=%d)\n", wait_ms, wait_rc);

if (wait_rc == 0) {
printk("MQTT publish successful: topic=%s qos=%d (total %lld ms)\n",
       topic, p.message.topic.qos, k_uptime_get() - start_ms);
return 0;
}

printk("MQTT publish failed: topic=%s qos=%d (total %lld ms, rc=%d)\n",
       topic, p.message.topic.qos, k_uptime_get() - start_ms, wait_rc);
return wait_rc;
}
...
int64_t start_ms = k_uptime_get();
printk("[MQTT-TIME] ping start @ %lld ms uptime\n", start_ms);

int rc = mqtt_ping(&mt_mqtt_client);
int64_t tx_ms = k_uptime_get() - start_ms;
printk("[MQTT-TIME] ping tx: %lld ms\n", tx_ms);

if (rc < 0) {
printk("[MQTT-TIME] PINGRESP wait: %lld ms (rc=%d)\n",
       k_uptime_get() - start_ms - tx_ms, rc);
rc = mqtt_disconnect(&mt_mqtt_client, NULL);
printk("[MT-POLL] MQTT PING disconnect: %d\n", rc);
printk("MQTT ping failed: (total %lld ms, rc=%d)\n",
       k_uptime_get() - start_ms, rc);
} else {
printk("[MQTT-TIME] PINGRESP wait: %lld ms (rc=%d)\n",
       k_uptime_get() - start_ms - tx_ms, rc);
printk("MQTT ping successful: (total %lld ms)\n",
       k_uptime_get() - start_ms);
...
if (!mt_fw_version_queried) {
rc = mt_query_fw_version();
if (rc != 0) {
printk("[MT-FLOW] WiFi version query failed, retrying before connect\n");
k_sleep(K_SECONDS(2));
continue;
}

mt_fw_version_queried = true;
}
