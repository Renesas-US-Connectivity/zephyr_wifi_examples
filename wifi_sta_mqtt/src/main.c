...
static void net_event_handler(struct net_mgmt_event_callback *cb,
  ...
  case NET_EVENT_IPV4_DHCP_BOUND:
    k_event_set(&net_event, mgmt_event);
    struct in_addr *addr4 =
        net_if_ipv4_get_global_addr(iface, NET_ADDR_PREFERRED);
    if (addr4) {
      LOG_INF("IPv4 address: %s",
              net_addr_ntop(AF_INET, addr4, buf, sizeof(buf)));
    }
    break;

  case NET_EVENT_IPV6_ADDR_ADD:
    k_event_set(&net_event, NET_EVENT_IPV6_ADDR_ADD);
    struct net_if *target_iface = iface;
    struct in6_addr *addr6 =
        net_if_ipv6_get_global_addr(NET_ADDR_PREFERRED, &target_iface);
    if (addr6) {
      LOG_INF("IPv6 address: %s",
              net_addr_ntop(AF_INET6, addr6, buf, sizeof(buf)));
    }
    break;
