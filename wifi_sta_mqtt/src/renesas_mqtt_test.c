...
K_THREAD_DEFINE(mqtt_thread,
		K_THREAD_STACK_LEN(CONFIG_MQTT_THREAD_STACK_SIZE),
		mqtt_comm_thread,
		&client, NULL, NULL,
...
static void mqtt_publish_data(void)
{
	...
	LOG_HEXDUMP_DBG(param.message.payload.data, param.message.payload.len, "pub message");

	ret = mqtt_publish(&client, &param);
	LOG_ERR("TEST MQTT PUBLISH: %d (%s)", ret, strerror(-ret));
}

static ssize_t handle_published_message(struct mqtt_client *client, const struct mqtt_publish_param *pub)
{
	int ret;
	size_t received = 0u;
	...
	LOG_INF("RECEIVED on topic \"%.*s\" [ id: %u qos: %u ] payload: %u B",
		pub->message.topic.topic.size, (const char *)pub->message.topic.topic.utf8, pub->message_id,
		pub->message.topic.qos, message_len);

	do {
		ret = mqtt_read_publish_payload_blocking(client, &payload_buf[received], message_len - received);
		if (ret == 0) {
			break;
		}
	...
	case MQTT_QOS_2_EXACTLY_ONCE:
		mqtt_publish_qos2_receive(client, &(struct mqtt_pubrec_param) { pub->message_id });
		break;
	default:
		break;
	}
...
#ifdef MQTT_POLL_WORKAROUND
		rc = mqtt_ping(client);
		LOG_ERR("TEST MQTT PING: %d (%s)", rc, strerror(-rc));
#endif
	}
}
